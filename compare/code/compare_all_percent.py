...
# 加载模型
path_model_a = "ckpt/Qwen2.5-32B-Instruct"
path_model_b = "ckpts/qwen_s1K_tokenized_bs16_lr1e-5_epoch5_wd1e-4_20250415_114516"
...
for name in state_dict_a:
    if name in state_dict_b:
        ...
        if match:
            ...
            epsilon = 1e-8  # 防止除零
            diff_matrix = (w_b - w_a) / (w_a.abs() + epsilon)
...
for (layer_idx, block_type, proj_name), diff_matrix in tqdm(layer_diffs.items(), desc="Drawing heatmaps"):
    ...
    # 裁剪异常值
    clip_threshold = 1.0
    sample = sample.clamp(min=-clip_threshold, max=clip_threshold)
    ...
    # 保存高分辨率图
    filename = f"compare/result/s1/percent/Layer{layer_idx}_{block_type}_{proj_name}_diff_with_hist.jpg"
